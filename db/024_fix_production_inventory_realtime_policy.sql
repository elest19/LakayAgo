-- Ensure the production_inventory catalog table is fully wired for Supabase Realtime.
-- 019_supabase_realtime_rollout.sql added production_inventory to the publication but
...
--    Intentionally permissive, matching the 019/022 realtime policy pattern; sensitive
...
-- select schemaname, tablename from pg_publication_tables where pubname = 'supabase_realtime' and tablename = 'production_inventory';
-- select schemaname, tablename, policyname from pg_policies where schemaname = 'public' and tablename = 'production_inventory';