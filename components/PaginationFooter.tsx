...
export default function PaginationFooter({ items, page, setPage, pageSize = DEFAULT_PAGE_SIZE, noun = 'items' }: Props) {
  const totalPages = Math.max(1, Math.ceil((items?.length || 0) / pageSize))

  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-white rounded-b-xl">
      <p className="text-xs text-slate-500">Showing {items.length === 0 ? 0 : (page - 1) * pageSize + 1}–{Math.min(page * pageSize, items.length)} of {items.length} {noun}</p>
