...
export async function GET(req: Request) {
  try {
    ...
    const rows = await query("SELECT id, undertime_deduction, undertime_deduction_rate_type, undertime_deduction_rate, created_at, updated_at FROM payroll_settings ORDER BY updated_at DESC LIMIT 1")
